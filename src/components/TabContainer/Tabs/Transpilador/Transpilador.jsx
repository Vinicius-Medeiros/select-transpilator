...
const Transpilador = () => {
    ...
    const formatErrorMessage = error => {
        if (!error.expected || !error.location)
            return "Erro desconhecido na consulta SQL.";
                ...
                tables.forEach(table => {
                    ...
                    ast.columns.forEach(column => {
                        if (column.expr.type === "column_ref") {
                            ...
                            if (
                                !!!Tabelas[tableRef.toLowerCase()] ||
                                !!!Tabelas[tableRef.toLowerCase()][field]
                            ) {
                                setIsSqlCommandValid(false);
                                throw new Error(
                                    `O campo '${field}' não existe na tabela '${tableRef}'.`
                                );
                            }
                        }
                    });
        ...
        <Grid container spacing={2} marginTop={2}>
            <Grid size={{ md: 12 }}>
                <Typography sx={{ width: "100%", textAlign: "center" }}>
                    É necessário validar o comando SQL para poder fazer a
                    transpilação!
                    <br />
                    Para conseguir fazer a transpilação o comando SQL precisa
                    estar correto.
                </Typography>
            </Grid>
            <Grid size={{ md: 5.5 }}>
            ...
            >
                <Button
                    id="button-validar-sql"
                    variant="contained"
                    onClick={handleValidarConsultarSql}
                >
                    VALIDAR
                </Button>
                <Button
                    id="button-transpilate-sql"
                    variant="contained"
                    disabled={!isSqlCommandaValid}
                    onClick={handleTransiplateSqlCommand}
                >
                    TRANSPILAR
                </Button>
            </Grid>
