...
const TabContainer = () => {
    ...
    return (
        <TabContext value={value}>
            <TabList onChange={handleChangeTab} sx={tabListStyle}>
                {tabList.map(tab => tab.component)}
            </TabList>
            {tabs.map(tab => (
                <TabPanel key={"tabPanel - " + tab.value} value={tab.value} sx={tabPanelStyle}>
                    <Tabs indicatorColor="white" sx={tabsStyle} value={false}>
                        {tab.tabContent}
                    </Tabs>
                </TabPanel>
            ))}
